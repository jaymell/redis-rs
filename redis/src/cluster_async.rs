...
#[derive(Clone)]
pub struct Connection<C = crate::aio::MultiplexedConnection>(mpsc::Sender<Message<C>>);
...
enum CmdArg<C> {
    Cmd {
        cmd: Arc<Cmd>,
        func: fn(C, Arc<crate::Cmd>) -> RedisFuture<'static, Response>,
    },
    Pipeline {
    ...
    fn slot(&self) -> Option<u16> {
        fn get_cmd_arg(cmd: &Cmd, arg_num: usize) -> Option<&[u8]> {
            cmd.args_iter().nth(arg_num).and_then(|arg| match arg {
                crate::Arg::Simple(arg) => Some(arg),
                crate::Arg::Cursor => None,
            })
        }
    ...
    C: ConnectionLike + Connect + Clone + Send + Sync + 'static,
{
    ...
    fn try_request(
        &mut self,
        info: &RequestInfo<C>,
    ) -> impl future::Future<Output = (String, RedisResult<Response>)> {
...
impl Connect for crate::aio::MultiplexedConnection {
    fn connect<'a, T>(info: T) -> crate::RedisFuture<'a, crate::aio::MultiplexedConnection>
    where
        T: IntoConnectionInfo + Send + 'a,
    {
        async move {
...
fn get_password(addr: &str) -> Option<String> {
    crate::parse_redis_url(addr).and_then(|url| url.password().map(|s| s.into()))
}
