...
impl<C> CmdArg<C> {
    ...
    fn slot(&self) -> Option<u16> {
        fn get_slot(cmd: &Cmd) -> Option<(u16, u16)> {
        ...
        match self {
            Self::Cmd { ref cmd, .. } => get_slot(cmd).map(|x| x.0),
            Self::Pipeline { ref pipeline, .. } => {
                let mut iter = pipeline.cmd_iter();
                let slot = iter.next().map(get_slot)?;
                for cmd in iter {
                    if slot != get_slot(cmd) {
                        return None;
                    }
                }
                slot.map(|x| x.0)
    ...
    ) -> impl Future<Output = Result<(SlotMap, ConnectionMap<C>), (RedisError, ConnectionMap<C>)>>
    {
        ...
        async move {
            ...
            for (_, conn) in connections.iter_mut() {
                ...
                match parse_slots(value, use_tls).and_then(|v| Self::build_slot_map(v)) {
                    Ok(s) => {
                        result = Ok(s);
                        break;
                    }
